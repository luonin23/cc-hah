              ...
              {sessions
                ...
                .map((session) => (
                  <button
                    key={session.id}
                    onClick={() => {
                      useTabStore.getState().openTab(session.id, session.title || 'Untitled')
                      setActiveView('code')
                      connectToSession(session.id)
                    }}
                    className="flex w-full items-center gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-container)] px-4 py-3 text-left transition-colors hover:bg-[var(--color-surface-hover)]"
                  >
                    <span className="material-symbols-outlined shrink-0 text-[18px] text-[var(--color-text-tertiary)]">chat_bubble</span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-[var(--color-text-primary)]">
                        {session.title || 'Untitled Session'}
                      </p>
                      <p className="text-xs text-[var(--color-text-tertiary)]">
                        {session.messageCount} 条消息 · {formatTimeAgo(session.modifiedAt)}
                      </p>
                    </div>
                    <span className="material-symbols-outlined shrink-0 text-[16px] text-[var(--color-text-tertiary)]">arrow_forward</span>
                  </button>
                ))}
            </div>
          </div>
