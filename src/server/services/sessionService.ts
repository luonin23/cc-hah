...
export class SessionService {
  ...
  private async readJsonlFile(filePath: string, maxLines?: number): Promise<RawEntry[]> {
    try {
    ...
    // Stream read for large files (>5MB) to avoid memory spikes
    const useStream = statSync(filePath).size > 5_000_000
