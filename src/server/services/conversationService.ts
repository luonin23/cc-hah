...
export class ConversationService {
  ...
  async startSession(
    ...
    // FIX: Kill stale CLI process before starting new one
    if (this.sessions.has(sessionId)) {
      const existing = this.sessions.get(sessionId)!
      if (existing.proc.pid) {
        console.log(
          `[ConversationService] Killing existing CLI process (pid=${existing.proc.pid}) for ${sessionId} before restart`,
        )
        existing.proc.kill()
      }
      this.sessions.delete(sessionId)
    } else {
      // FIX: Kill zombie CLI processes from a previous server restart.
      // The CLI connects back via SDK WebSocket — if the server restarted those
      // old processes still try to reconnect with stale tokens, causing the
      // "Rejected SDK connection" storm. Kill them by session ID in the ps tree.
      try {
        const { execSync } = require('child_process')
        const escapedId = sessionId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
        const ps = execSync(
          `pgrep -f "cli\\.tsx.*(\\-\\-resume|\\-\\-session\\-id)\\s+${escapedId}"`,
        ...
        if (ps) {
          for (const pid of ps.split('\n').filter(Boolean)) {
            const p = pid.trim()
            console.log(
              `[ConversationService] Killing zombie CLI process (pid=${p}) for ${sessionId}`,
            )
            try { process.kill(parseInt(p, 10), 'SIGKILL') } catch {}
          }
        }
      } catch { /* pgrep not available or timed out */ }
    }
  ...
  stopSession(sessionId: string): void {
    const session = this.sessions.get(sessionId)
    if (session) {
      if (session.sigkillTimer) {
        clearTimeout(session.sigkillTimer)
        session.sigkillTimer = undefined
      }
      try {
      ...
      session.sigkillTimer = setTimeout(() => {
        session.sigkillTimer = undefined
        try {
