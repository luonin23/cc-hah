...
/** Server-side heartbeat timers — keep browser WebSocket alive through NAT/router timeouts */
const serverHeartbeatTimers = new Map<string, ReturnType<typeof setInterval>>()
const HEARTBEAT_INTERVAL_MS = 30_000 // 30秒 ping 一次
...
/** Force-kill timers after stop-generation — must be cleared on new message or cleanup */
const stopGenerationTimers = new Map<string, ReturnType<typeof setTimeout>>()
...
const DEFAULT_PREWARM_IDLE_TIMEOUT_MS = 30 * 60_000 // 30分钟
...
export const handleWebSocket = {
  open(ws: ServerWebSocket<WebSocketData>) {
    ...
    // Start server-side heartbeat to keep browser WebSocket alive
    startServerHeartbeat(sessionId, ws)
  ...
  message(ws: ServerWebSocket<WebSocketData>, rawMessage: string | Buffer) {
    ...
    try {
      ...
      switch (message.type) {
        ...
        case 'client_pong':
          // Client responded to server heartbeat — connection is alive
          break
  ...
  close(ws: ServerWebSocket<WebSocketData>, code: number, reason: string) {
    ...
    // Cancel server heartbeat timer
    const heartbeat = serverHeartbeatTimers.get(sessionId)
    if (heartbeat) {
      clearInterval(heartbeat)
      serverHeartbeatTimers.delete(sessionId)
    }

    // Schedule delayed cleanup: if the client doesn't reconnect within the grace period,
    // stop the CLI subprocess to free memory. Client can resume from history list.
    const disconnectGraceMs = Number.parseInt(process.env.CC_HAHA_DISCONNECT_GRACE_MS || '300000', 10)
    const cleanupDelay = Number.isFinite(disconnectGraceMs) && disconnectGraceMs >= 0 ? disconnectGraceMs : 300_000
    const cleanupTimer = setTimeout(() => {
      sessionCleanupTimers.delete(sessionId)
      if (!activeSessions.has(sessionId)) {
        console.log(`[WS] Session ${sessionId} not reconnected after ${cleanupDelay}ms, stopping CLI subprocess to free memory`)
...
async function handleUserMessage(
  ...
  clearStopGenerationTimer(sessionId)
...
function handleStopGeneration(ws: ServerWebSocket<WebSocketData>) {
  ...
  if (conversationService.hasSession(sessionId)) {
    ...
    const forceKillTimer = setTimeout(() => {
      stopGenerationTimers.delete(sessionId)
      if (conversationService.hasSession(sessionId)) {
        console.log(`[WS] Force-killing CLI subprocess for session: ${sessionId}`)
        conversationService.stopSession(sessionId)
      }
    }, 3_000)
    stopGenerationTimers.set(sessionId, forceKillTimer)
  }
...
function clearStopGenerationTimer(sessionId: string) {
  const timer = stopGenerationTimers.get(sessionId)
  if (timer) {
    clearTimeout(timer)
    stopGenerationTimers.delete(sessionId)
  }
...
function startServerHeartbeat(sessionId: string, ws: ServerWebSocket<WebSocketData>) {
  // Clear existing timer if any (e.g. reconnect)
  const existing = serverHeartbeatTimers.get(sessionId)
  if (existing) clearInterval(existing)

  const timer = setInterval(() => {
    const currentWs = activeSessions.get(sessionId)
    if (currentWs !== ws) {
      // Client disconnected or replaced — stop heartbeat
      clearInterval(timer)
      serverHeartbeatTimers.delete(sessionId)
      return
    }
    try {
      currentWs.send(JSON.stringify({ type: 'server_ping' }))
    } catch {
  ...
  serverHeartbeatTimers.set(sessionId, timer)
}
